...
简报标题会显示已验证的客户展示名和模型范围，例如“佛跳墙 Kimi-K3模型日报简报”；多个模型统一显示“全部模型”。所有用量简报隐藏时间桶、桶数量和 interval；详细分析和健康诊断仍保留必要的趋势口径。结果卡不再提供订阅按钮，已有订阅继续执行。新建订阅可通过受控自然语言、引用飞书报表卡片或 WebUI `Report platform` 完成；订阅保存实时 catalog 验证后的真实 `tenantId`。

引用订阅只从服务端保存的报表消息引用恢复模板、客户和模型范围，不从卡片文本反向猜测。LLM 仅提取发送计划和用户明确修改的名称；服务端重新执行 catalog 解析、RBAC、订阅策略和幂等校验，并在创建前显示确认卡。历史查询日期不会进入周期订阅。

直接自然语言订阅同样先从原文提取完整客户列表，再与 Cube 实时目录逐一合并；例如“每天上午十点发送阳春面、豆汁、佛跳墙全部模型的多客户日报简报”必须在确认卡中保留三个真实客户，并使用 `usage_customer_model_daily_brief`。LLM 不生成 `tenantId`、Cron、URL、API 路径或工具参数。引用主卡或续卡时，范围由服务端安全引用记录恢复，当前用户仍需重新通过权限校验。
...
| 多客户多模型日报简报 | 昨日；同比 D-7、环比 D-1 | 按客户分组的模型 Token 总数、同比和环比百分比；全部模型模式过滤三个窗口均无用量的模型 | 独立简报，默认关闭 |
| 多客户多模型周报简报 | 上一完整自然周 vs 前一完整自然周 | 按客户分组的模型 Token 总数和环比百分比 | 独立简报，默认关闭 |
...
- 独立控制结果卡片是否显示订阅按钮（`show_subscription_button`）；隐藏按钮不等于禁止订阅，`subscription_mode=disabled` 才阻止新建订阅。
- 分页查看订阅，并创建、修改计划、启用、停用和删除；变更同步 Cron。
- 引导式表单提供报表、客户、模型、周期、频率、时间、时区和接收会话字段；用户不需要手填 Cron 或 `report_params_json`。旧接口保留兼容，但新建和编辑优先走统一订阅服务。
...
订阅报表支持两种方式：直接描述客户、模型和发送计划，或引用已有报表卡片后只描述发送计划。确定性短语（如“每天上午十点”或“工作日上午十点”）优先本地解析；表达不完整时最多进行一次结构化解析。引用卡片的客户、模型和模板范围必须来自服务端安全引用记录，不能从卡片文字或模型猜测。

多客户全部模型日报始终使用 `usage_customer_model_daily_brief`，每次执行前重新读取客户的实时模型目录。当前测试账号的订阅接收人仍限制为当前用户。

当引用记录缺失、过期、跨会话或当前用户无权限时，提示引用范围不可恢复；当引用记录存在但发送计划解析失败时，提示发送计划无法识别。两类错误不能混用，也不能把失败转换为空报表或零值订阅。
