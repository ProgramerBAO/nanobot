...
| 多客户多模型小时 TPM 报告 | 以六列表格展示上一完整小时各模型数据：客户、模型、TPM 峰值、均值、机器占用、机器真实使用，卡尾附七列集群机器库存表（集群/机器总数/生产/测试/开发/备用/空闲，生产=总数−其余之和）；订阅支持每小时（`5 * * * *`）与指定小时列表（如 `5 9,10 * * *`），整点后 5 分钟投递上一完整小时 | 模板策略（`usage_customer_model_hourly_tpm`，订阅受众由同模板 `subscription_mode` 控制） |
...
集群机器库存来自 `analysis/machine-usage-summary/query`（POST `{"noloading":true}`，发送时平台级快照，`occupiedMachineCount` 语义为测试机总数）：七列表格（集群 | 机器总数 | 生产 | 测试 | 开发 | 备用 | 空闲）按集群名排序，生产 = 机器总数 − 测试 − 开发 − 备用 − 空闲（计算列，分项缺失或之和超过总数时显示 —，后者同时产生信息性告警）。整表为信息性补充：接口失败或空列表不产出该表、不降级质量；集群级空闲与模型级（闲N）/副标题机器空闲口径不同、互不换算。纯文本 fallback 以单行"集群空闲机器"摘要呈现（缺失空闲数的集群不入行）。
...
- 小时卡片第 3 张卡为集群机器库存表：按集群名排序的七列（集群 | 机器总数 | 生产 | 测试 | 开发 | 备用 | 空闲），生产 = 总数−其余之和；machine-usage-summary 失败或空列表时不产出该表、整卡质量不降级，仅折叠说明中出现信息性告警。
