...
小时播报的指定小时：`hours` 为 0-23 的小时列表（去重排序、1-24 个），编译为 `5 9,10 * * *`；`"每小时"`保持全 24 小时（`5 * * * *`）。执行窗口始终由运行时钟推导上一完整小时，与 cron 表达式无关。`hours` 仅对小时 TPM 报表有效（recent1h 与 hourly 双向绑定，其他组合拒绝）；工作日/每周/每月与小时列表的组合不支持（确定性解析放弃、不做静默周末投递）。快照反推：分钟 5 + 小时列表（≥2）或小时 TPM 模板上下文中的单小时判定为 hourly，避免编辑时退化为每天 HH:05。
