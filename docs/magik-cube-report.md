...
用量简报结果不再显示订阅按钮。已有订阅和 Cron 执行链路保持不变；新建订阅可以直接使用自然语言、引用一张已生成的报表卡片，或在 WebUI `Report platform` 中完成。订阅参数仍保存 Cube catalog 验证后的真实 `tenantId`，历史查询日期不会固化到周期订阅中。

WebUI 的引导式订阅表单（`report_subscription_guided_ui`）提供报表类型、客户、模型、周期、频率、发送时间、时区和接收会话选项；用户不需要填写 Cron 或原始 `report_params_json`。管理员可以在 `Report platform -> 报表类型` 中分别控制模板是否启用、是否允许新订阅，以及结果卡片是否显示订阅按钮。`show_subscription_button=false` 只影响展示，不会停用已有订阅；`subscription_mode=disabled` 才会拒绝新建订阅。

自然语言订阅示例：

> 每天上午十点发送阳春面、豆汁、佛跳墙全部模型的多客户多模型日报简报

引用订阅时，在同一飞书群或私聊中引用日报、周报或月报卡片并回复：

> 工作日上午十点发送给我

系统从服务端保存的 `message_id -> ReportIntent scope` 恢复模板、客户和模型范围，LLM 只解析发送计划与用户明确修改的范围。创建前必须经过实时 catalog、RBAC、模板订阅策略校验并显示确认卡。引用旧卡片、普通文本、跨群消息或过期记录时不会猜测范围，而是提示重新生成报表或进入订阅中心。

多客户“全部模型”订阅每次执行前重新读取各客户实时模型目录，因此后续新增模型会自动纳入；持续无用量模型继续按简报规则隐藏。引用历史日报只继承日报语义，不会每天重复发送历史日期的数据。

直接自然语言订阅会先识别原文中的全部客户，再与实时 Cube catalog 合并。例如：

```text
每天上午十点发送阳春面、豆汁、佛跳墙全部模型的多客户日报简报
```

确认卡必须展示三个客户、全部模型和当前测试账号；只有点击确认后才创建一个绑定当前用户的订阅。引用报表卡片时，范围来自服务端保存的安全消息引用，不能从卡片文字猜测；跨会话、过期或旧卡片无法恢复时会进入重新选择流程。
...
相关发布开关：`cube_subscription_nlu_v2` 控制直接自然语言解析，`cube_subscription_nlu_v3` 是增强解析灰度开关，`cube_report_reference_subscription` 控制引用卡片范围恢复；`report_management_v1` 和 `report_subscription_guided_ui` 控制管理台及引导式订阅编辑。上述开关关闭时保留旧兼容路径，不迁移或删除已有订阅。

### 多客户多模型日报简报

发送 `多客户多模型日报简报` 后选择客户和模型。报告按客户分组，每个模型展示当前日报 Token 总数，以及 Token `同比`（D-7）和 `环比`（D-1）百分比。发送 `多客户多模型周报简报` 时，报告使用上一完整自然周对比此前一完整自然周，仅显示环比。客户名来自实时 catalog 或已验证别名，查询始终使用真实 `tenantId`。“全部模型”会先通过 Cube 实时 `inference/model-configs` 目录展开为该客户的明确模型列表，再逐个查询；目录为空或读取失败时返回明确错误，不生成 `0 个模型` 的空壳报表。日报三个命名窗口均无 Token 用量的目录模型会自动隐藏；任一基准期有量、用户明确点选或存在查询失败时不隐藏。手工多选最多 20 个模型；“全部模型”允许使用实时目录中的完整模型集合，但仍受最多 20 个客户和 200 个客户模型组合的总上限保护。日报由 `cube_multi_scope_brief` 控制，周报由 `cube_multi_scope_weekly_brief` 控制，两个能力默认关闭。
...
明确的订阅短语会先使用确定性解析。例如“每天上午十点发送阳春面、豆汁、佛跳墙全部模型的多客户日报简报”会从实时 Cube 客户目录恢复三个真实租户，并生成 `usage_customer_model_daily_brief` 的确认卡；多客户周报引用或明确指定周报时使用 `usage_customer_model_weekly_brief`，不会退化为只包含一个客户的普通周期报表。

回复已生成的日报卡片“工作日上午十点发送给我”时，系统先通过服务端报表引用恢复原客户、模型和模板范围，再解析发送计划。历史报表日期不会成为订阅的固定日期。只有无法确定的表达才调用一次结构化 LLM，LLM 不生成 tenant ID、Cron、URL 或 API 参数。

错误提示区分为：引用不存在或过期、订阅计划无法解析、Cube 客户目录不可用、客户名称歧义和客户范围未确认。解析失败不会提示用户重新生成一张已经存在的报表卡片，也不会创建缩小后的订阅。
