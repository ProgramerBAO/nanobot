...
`功能开关` 页管理 10 项运行时功能开关（用量报表、订阅、成本报表、管理界面四组）：简报默认路由、Cube 灵活查询帮助、Cube 报表订阅、自然语言订阅、引用报表创建订阅、成本报表、成本报表订阅、Report platform 管理页、引导式订阅编辑、订阅按钮策略。每项开关的持久化覆盖写入 store 的 `report_feature_flags` 表并记录管理审计；覆盖优先于部署配置默认值，未覆盖时回落默认（成本报表两项默认关闭，其余默认开启）。切换即时生效，无需重启；已覆盖的行可"恢复默认值"（删除覆盖行）。

每个报表模板的启用/停用**不在**本页：它由 `报表类型` 页签的模板策略管理（始终执行，`enabled` 同时管执行、可见与新建订阅）。2026-09-16 收敛中被退役的每模板运行时开关（健康报告、供应商质量、多客户简报、小时 TPM、单机 TPM 等 11 项）已迁入模板策略；升级后运行一次 `nanobot reports policy migrate-flags` 即可把原有覆盖与配置级非默认值转换为模板策略行。
